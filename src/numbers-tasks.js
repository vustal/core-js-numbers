...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const modulusX = Math.hypot(x1, y1);
  const modulusY = Math.hypot(x2, y2);
  return Math.acos(dotProduct / (modulusX * modulusY));
}
...
function getFibonacciNumber(index) {
  ...
  const goldenRatioConjugated = (1 - Math.sqrt(5)) / 2;
  const fiNumber =
    (goldenRatio ** index - goldenRatioConjugated ** index) / Math.sqrt(5);
  return Math.round(fiNumber);
}
